...
class TfPrCurveDashboard extends LegacyElementMixin(PolymerElement) {
  ...
  _fetchTags() {
    const url = getRouter().pluginRoute('my_pr_curves', '/tags');
  ...
  _tagMetadata(runToTagsInfo, runs, tag) {
    ...
    // All PR curve tags include the `/my_pr_curves` suffix. We can trim
    // that from the display name.
    const defaultDisplayName = tag.replace(/\/my_pr_curves$/, '');
