...
def main() :
    ...
    for (iCat, cat) in enumerate(d_catInfo.keys()) :
        ...
        for (iTree, treeName) in enumerate(catInfo.l_fileAndTreeName) :
            ...
            for idx in range(0, len(l_eventIdx)-1) :
                ...
                #l_d_arrayIdx.append(sortedcontainers.SortedDict())
                l_d_arrayIdx.append(multiprocessing.Manager().dict())
                ...
                l_isJobReady.append(False)
                l_isJobProcessed.append(False)
                
                
                while(
                    (len(l_isJobProcessed) - sum(l_isJobProcessed)) >= 30 or
                    (iTree == len(catInfo.l_fileAndTreeName)-1 and idx == len(l_eventIdx)-2 and False in l_isJobProcessed)
                ) :
                    ...
                    for iJob, job in enumerate(l_job) :
                        ...
                        #print(iJob, len(l_d_arrayIdx))
                        
                        l_isJobReady[iJob] = job.ready()
                        
                        if (not l_isJobProcessed[iJob] and l_isJobReady[iJob]) :
                            
                            #nJet_sel, d_arrayIdx_temp = job.get()
                            nJet_sel = job.get()
                            #d_arrayIdx_temp = sortedcontainers.SortedDict(l_d_arrayIdx[iJob])
                            d_arrayIdx_temp = l_d_arrayIdx[iJob]
                            print("#"*20, len(d_arrayIdx_temp))
                            
                            print("Finished read-job num %d (%d/%d done) of category %d." %(iJob+1, sum(l_isJobReady), len(l_job), catNum))
                            ...
                            print("$"*10, "len(l_sparseArr_idx), len(l_sparseArr_val), nJet_cat, jetIdx:", len(l_sparseArr_idx), len(l_sparseArr_val), nJet_cat, jetIdx)
                            print("$"*10, "size(l_sparseArr_idx), size(l_sparseArr_val) in GB:", sys.getsizeof(l_sparseArr_idx)/1024.0**3, sys.getsizeof(l_sparseArr_val)/1024.0**3)
                            
                            #d_sortedIdx.update(d_arrayIdx_temp1)
                            
                            #d_arrayIdx_temp.clear()
                            #nJet_sel, d_arrayIdx_temp = job.get()
                            #print("#"*20, len(d_arrayIdx_temp))
                            ...
                            l_job[iJob] = None
                            l_isJobProcessed[iJob] = True
                            
                            l_d_arrayIdx[iJob].clear()
                            l_d_arrayIdx[iJob] = None
                            
                            if (not sum(l_isJobProcessed) % 10) :
                                
                                gc.collect()
                            
                            
                            print("Processed output of read-job num %d (%d/%d done) of category %d." %(iJob+1, sum(l_isJobProcessed), len(l_job), catNum))
                            print("=====> Memory:", getMemoryMB())
